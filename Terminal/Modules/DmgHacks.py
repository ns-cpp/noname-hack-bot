...
class DmgHacksInstance(ui.Window):
	...
	def BuildWindow(self):
		...
		self.enableButton = self.comp.OnOffButton(self.Board, '', '', 130, 210, OffUpVisual=eXLib.PATH + 'Terminal/Images/start_0.tga', OffOverVisual=eXLib.PATH + 'Terminal/Images/start_1.tga', OffDownVisual=eXLib.PATH + 'Terminal/Images/start_2.tga',OnUpVisual=eXLib.PATH + 'Terminal/Images/stop_0.tga', OnOverVisual=eXLib.PATH + 'Terminal/Images/stop_1.tga', OnDownVisual=eXLib.PATH + 'Terminal/Images/stop_2.tga',funcState=self.OnOffBtnState )
	...
	def TeleportAttack(self,lst,x,y):
		...
		for vid in lst:
			...
			if OpenLib.dist(x,y,mob_x,mob_y) < OpenLib.ATTACK_MAX_DIST_NO_TELEPORT:
				#chat.AppendChat(3,str(OpenLib.dist(x,y,mob_x,mob_y)))
				main_vid = player.CanActMainInstance()
				if vid !=0 and main_vid != 0:
	...
	def OnUpdate(self):
		...
		main_vid = player.GetMainCharacterIndex()
		if not player.IsDead():
			...
			for vid in inst_list:
				...
				if player.GetCharacterDistance(vid) < self.range and not player.IsDead():
					lst.append(vid)
						

			hit_counter = 0
			i = 0
			while len(lst) > 0 and hit_counter < 6:
				vid = lst[0]
				mob_x, mob_y, mob_z = chr.GetPixelPosition(vid)
