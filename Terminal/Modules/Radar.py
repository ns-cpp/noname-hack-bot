...
import chat, ui, chr, net, background,player
import OpenLib
import eXLib
import re
...
class Radar(BotBase):
    ...
    def BuildWindow(self):
        ...
        self.enableButton = comp.OnOffButton(self.settings_tab, '', '', 15, 40,
                                             ...
                                             funcState=self._start, defaultValue=True)
	...
	dis=player.GetCharacterDistance(vid)
        entity = {
            'type': instance_type,
            'vid': vid,
            'x': x,
	...
	if name != None:
		with open(eXLib.PATH+"\\dump.txt","a") as file :
   		 file.write(name+"\n")
		with open(eXLib.PATH+"\\dump1.txt","a") as file :
   	 	 file.write(str(dis)+"\n")
    ...
    def Frame(self):
        MAIN_CHAR_VID = net.GetMainActorVID()
        
        self.all_vids = eXLib.InstancesList
        val, Data.time_Radar_lastTimeClearedList = OpenLib.timeSleep(Data.time_Radar_lastTimeClearedList, 0.1)
        if val:
            self.clear_lists()
    ...
    def clear_lists(self):
        self.fileListBoxEntities.RemoveAllItems()
	with open(eXLib.PATH+"\\dump.txt","w") as file :
   	 file.write("")
	with open(eXLib.PATH+"\\dump1.txt","w") as file :
   	 file.write("")
        self.entities = []
