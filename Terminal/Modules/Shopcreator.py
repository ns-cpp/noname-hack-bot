import ui,app,chat,chr,net,player,item,skill,time,game,shop,chrmgr,player,eXLib
...
class ShopDialog(ui.ScriptWindow):
	...
	def SetItemPrice(self,slot):
		...
		try:
			item_info = {}
			with open(eXLib.PATH+"//datesystem.txt", "r") as file:
				for line in file:
					index, price = line.strip().split(":")
					item_info[int(index)] = {
					"price": price
			...
			_id = player.GetItemIndex(slot)
			if _id in item_info:
				str_price = int(item_info[_id]["price"])

				yang = str_price
				adet = int(player.GetItemCount(slot))
				yang = yang * adet
				...
				if(wons > 0):
					...
					rest_yang = int(yang)
				if wons + rest_yang <= 0.9: 
					return

				shop.AddPrivateShopItemStock(player.SLOT_TYPE_INVENTORY,slot,slot,rest_yang,wons)
		except:
			pass
	...
	def CreateShop(self):
		...
		for i in range(0, self.MAX_NUM_SLOT):
			...
			#if ItemValue != 0:
			self.SetItemPrice(i)
		shop.BuildPrivateShop(self.nameshop)
