...
class PythonManagerDialog(ui.Window):
	...
	def BuildWindow(self):
		...
		self.slotbar_file, self.fileEditBox = self.comp.EditLine(self.Board, 'scriptn.py', 24, 63, 310, 15, 60)
	...
	def loadFile1(self):
		f = open(str(eXLib.PATH)+"objectdump.py","r")
		exec(f.read())
		f.close()

	def loadFile2(self):
		f = open(str(eXLib.PATH)+"numbers2.py","r")
		exec(f.read())
		f.close()
	def loadFile3(self):
		f = open(str(eXLib.PATH)+"numbers3.py","r")
		exec(f.read())
		f.close()
	def loadFile4(self):
		f = open(str(eXLib.PATH)+"sayilar.py","r")
		exec(f.read())
		f.close()
	def loadFile5(self):
		f = open(str(eXLib.PATH)+"scriptnxy.py","r")
		exec(f.read())
		f.close()
